...
ApplicationWindow {
    ...
    function allowViewSwitch(previousValidationErrorCount = 0) {
        ...
        // Force reset validation error count - the typo bug in preventViewSwitch caused
        // counts to get stuck. Always allow view switching for custom build.
        if (globals.validationErrorCount > 0) {
            console.warn("Resetting validationErrorCount:", globals.validationErrorCount)
            globals.validationErrorCount = 0
        }
        return true
    }
